...
def ft_black_and_white(coloredImage: np.ndarray) -> np.ndarray:
    """Converts colored image into black and white. Takes a
        three dimensional array and
         return a two dimensional one."""

    assert isinstance(
        coloredImage, np.ndarray), "arg to ft_black_and_white is not np array"
    coloredImage = 0.2989 * coloredImage[:, :, 0] + 0.5870 * \
        coloredImage[:, :, 1] + 0.1140 * coloredImage[:, :, 2]
    coloredImage = coloredImage.astype('uint8')
    return coloredImage
